...
local test_utils = require("atf/test_utils")
...
function module:waitForSDLStart(test)
  local step = 100
...
function module:createMultipleExpectationsWaiter(test, name)
...
function module:initHMI()
  local exp_waiter = self:createMultipleExpectationsWaiter(module, "HMI initialization")
...
function module:initHMI_onReady(hmi_table)
  local exp_waiter = self:createMultipleExpectationsWaiter(module, "HMI on ready")
  ...
  if type(hmi_table) == "table" then
    hmi_table_internal = cloneTable(hmi_table)
  else
